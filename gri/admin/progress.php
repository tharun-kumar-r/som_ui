...
if(isset($_POST['mov']))
{
  $sql = "UPDATE $posttype SET status='pending' WHERE id=$tkn";
...
if($check == "false")
{
 ...
 sendmail($mail,$pr="Moved To Pending",$useremail,$subset,$clgname);
}
...
SELECT id,dept,status,subject,des,data,data1,name,type,email,mobile,regno,filename,sem FROM publi_c WHERE status='inprogress' ORDER BY data DESC"; 
}
else
{
  $sql = "SELECT id,dept,status,subject,des,data,data1,name,type,email,mobile,regno,filename,sem FROM staff  WHERE status='inprogress' UNION ALL
  SELECT id,dept,status,subject,des,data,data1,name,type,email,mobile,regno,filename,sem FROM student WHERE status='inprogress' UNION ALL
  SELECT id,dept,status,subject,des,data,data1,name,type,email,mobile,regno,filename,sem FROM publi_c WHERE status='inprogress' ORDER BY data ASC"; 
...
{
  $screen .= '<tr><td>'. '<br><center><font color="#dc3545">No Inprogress Grievances Found!.</font></center><br>' . '</td></tr>';
}
          ...
          <div class="container-fluid">
            ...
            <nav aria-label="breadcrumb" role="navigation">
              <ol class="breadcrumb adminx-page-breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Inprogress Grievances</li>
              </ol>
            </nav>

            <div class="pb-3">
              <h1>Inprogress Grievances</h1>
            </div>
...
window.history.replaceState('1','th',url.toString());
   ...
   " checked> Send mail about this action</label>
     <form method="POST">
     <button class="xbtn1" name="mov">Move to pending</button>
     </form>
     <form method="POST">
